...
def eval_refcoco(task, generator, models, sample, **kwargs):
    def _calculate_ap_score(hyps, refs, thresh=0.5):
        ...
        interacts_w = torch.clamp(interacts[:, 2] - interacts[:, 0], min=0.0)
        interacts_h = torch.clamp(interacts[:, 3] - interacts[:, 1], min=0.0)
        area_interacts = interacts_w * interacts_h
        ious = area_interacts / (area_predictions + area_targets - area_interacts + 1e-6)
        if kwargs['ofa_test']:
            return ((ious >= thresh) & (interacts_w > 0) & (interacts_h > 0)).float()
        else:
            ious_mask = ious >= thresh
            return ious,ious_mask
    ...
    if kwargs['ofa_test']:
        scores = _calculate_ap_score(hyps, sample['region_coords'].float(), kwargs['threshold'])
        return results, scores
    else:
        scores, scores_mask = _calculate_ap_score(hyps, sample['region_coords'].float(), kwargs['threshold'])
        return results, scores, scores_mask
