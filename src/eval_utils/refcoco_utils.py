...
class REFER:
	...
	def __init__(self, dataset='refcoco', splitBy='unc', img_dir=None, ref_dir=None, verbose=True):
		...
		assert img_dir is not None, "Please assign MSCOCO train2014 image path to REFER class!"
		assert ref_dir is not None, "Please assign RefCOCO feature path to REFER class!"

		if type(ref_dir) == str:
			self.DATA_DIR = Path(ref_dir)
		else:
			self.DATA_DIR = ref_dir
